...
" spf13 options {
    ...
    " To set your own font, do it from ~/.config/nvim/local.vim
...
" Use fork before if available {
    if filereadable(expand("~/.config/nvim/before.fork.vim"))
        source ~/.config/nvim/before.fork.vim
    endif
" }

" Use local before if available {
    if filereadable(expand("~/.config/nvim/before.local.vim"))
        source ~/.config/nvim/before.local.vim
    endif
" }
